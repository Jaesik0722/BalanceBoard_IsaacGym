...
class BalanceBoardIK(VecTask):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        # Ideal Observations: 79 
        # Real World Possible Observations: 42
        ...
        self.filter_foot_position = FILTER_TORCH(self.num_envs, device=self.device)
        self.filter_foot_orientation = FILTER_TORCH(self.num_envs, device=self.device)
        self.filter_arm = FILTER_TORCH(self.num_envs, device=self.device)

        self.leg_position_upper = torch.ones((self.num_envs, 2), dtype=torch.float32, device=self.device) * 200.0
        self.leg_position_lower = torch.ones((self.num_envs, 2), dtype=torch.float32, device=self.device) * -200.0
        self.leg_orientation_upper = torch.ones((self.num_envs, 2), dtype=torch.float32, device=self.device) * (math.pi/2)
        self.leg_orientation_lower = torch.ones((self.num_envs, 2), dtype=torch.float32, device=self.device) * -(math.pi/2)
        self.arm_orientation_upper = torch.ones((self.num_envs, 4), dtype=torch.float32, device=self.device) * (math.pi/2)
        self.arm_orientation_lower = torch.ones((self.num_envs, 4), dtype=torch.float32, device=self.device) * -(math.pi/2)
        self.lpf_alpha_upper = torch.ones((self.num_envs, 3), dtype=torch.float32, device=self.device) * 0.1
        self.lpf_alpha_lower = torch.ones((self.num_envs, 3), dtype=torch.float32, device=self.device) * 0.0001
    ...
    def calculate_joint_position(self, leg_position: torch.Tensor, 
            leg_orientation: torch.Tensor, 
            arm_orientation: torch.Tensor, 
            lpf_alpha: torch.Tensor,
            env_ids: torch.Tensor):
        leg_x = leg_position[:, 0]
        leg_y = leg_position[:, 1]
        leg_roll = leg_orientation[:, 0]
        leg_pitch = leg_orientation[:, 1]
        arm_roll_left = arm_orientation[:, 0]
        arm_pitch_left = arm_orientation[:, 1]
        arm_roll_right = arm_orientation[:, 2]
        arm_pitch_right = arm_orientation[:, 3]
        lpf_alpha_foot_position = lpf_alpha[:, 0]
        lpf_alpha_foot_orientation = lpf_alpha[:, 1]
        lpf_alpha_arm = lpf_alpha[:, 2]
        
        # Add low pass filter for leg_x, leg_y, leg_roll, leg_pitch, arm
        leg_x = self.filter_foot_position.update_lowpass_filter(leg_x, lpf_alpha_foot_position, env_ids)
        leg_y = self.filter_foot_position.update_lowpass_filter(leg_y, lpf_alpha_foot_position, env_ids)
        leg_roll = self.filter_foot_orientation.update_lowpass_filter(leg_roll, lpf_alpha_foot_orientation, env_ids)
        leg_pitch = self.filter_foot_orientation.update_lowpass_filter(leg_pitch, lpf_alpha_foot_orientation, env_ids)
        arm_roll_left = self.filter_arm.update_lowpass_filter(arm_roll_left, lpf_alpha_arm, env_ids)
        arm_pitch_left = self.filter_arm.update_lowpass_filter(arm_pitch_left, lpf_alpha_arm, env_ids)
        arm_roll_right = self.filter_arm.update_lowpass_filter(arm_roll_right, lpf_alpha_arm, env_ids)
        arm_pitch_right = self.filter_arm.update_lowpass_filter(arm_pitch_right, lpf_alpha_arm, env_ids)
        
        leg_x_pos_left = leg_x
        leg_x_pos_right = leg_x

        leg_y_pos_left = leg_y
        leg_y_pos_right = leg_y

        leg_y_pos_left = leg_y_pos_left + ((HIP_LENGTH - (HIP_LENGTH * torch.cos(leg_roll))) / 2)
        leg_y_pos_right = leg_y_pos_right + ((HIP_LENGTH - (HIP_LENGTH * torch.cos(leg_roll))) / 2)

        leg_z_pos_left = torch.where(leg_roll <= 0, torch.abs(torch.sin(leg_roll) * HIP_LENGTH), 0.0)
        leg_z_pos_right = torch.where(leg_roll > 0, torch.sin(leg_roll) * HIP_LENGTH, 0.0)
        ...
        leg_values_left[LEG.ANKLE_PITCH.value] = leg_values_left[LEG.ANKLE_PITCH.value] - leg_pitch
        leg_values_left[LEG.ANKLE_ROLL.value] = leg_values_left[LEG.ANKLE_ROLL.value] - leg_roll

        leg_values_right[LEG.ANKLE_PITCH.value] = leg_values_right[LEG.ANKLE_PITCH.value] - leg_pitch
        leg_values_right[LEG.ANKLE_ROLL.value] = leg_values_right[LEG.ANKLE_ROLL.value] - leg_roll
    ...
    def reset_idx(self, reset_env_ids):
        self.filter_foot_position.reset_lowpass_filter(reset_env_ids)
        self.filter_foot_orientation.reset_lowpass_filter(reset_env_ids)
    ...
    def pre_physics_step(self, actions):
        ...
        leg_position = unscale_transform(actions[:, :2], self.leg_position_lower, self.leg_position_upper)
        leg_orientation = unscale_transform(actions[:, 2:4], self.leg_orientation_lower, self.leg_orientation_upper)
        arm_orientation = unscale_transform(actions[:, 4:8], self.arm_orientation_lower, self.arm_orientation_upper)
        lpf_alpha = unscale_transform(actions[:, 8:11], self.lpf_alpha_lower, self.lpf_alpha_upper)
        
        left_leg, right_leg, left_arm, right_arm = self.calculate_joint_position(
            leg_position,
            leg_orientation,
            arm_orientation,
            lpf_alpha,
    ...
    def _send_dof_targets(self, left_leg, right_leg, left_arm, right_arm):
        ...
        self.dof_target_tensor[:, 2] = self.torch_initial_pos[2] + left_arm_pitch
        self.dof_target_tensor[:, 6] = self.torch_initial_pos[6] + right_arm_pitch
        self.dof_target_tensor[:, 3] = self.torch_initial_pos[3] + left_arm_roll
        self.dof_target_tensor[:, 7] = self.torch_initial_pos[7] + right_arm_roll
